...
def pdf_generator_transactionlist (addressesList, app_instance_path):
    
        #filename = tempfile.NamedTemporaryFile( suffix='.pdf' ) 
        #cod = datetime.now()
        print("i have got addressesList" , addressesList)
        filename ="report.pdf"
        import os
        ...
        data = [
        ["ID Transaz", "Tipologia", "Data", "Blocco "]
        ]
        ...
        for asset in addressesList:
            ...
            # nome= asset['name']
            temp.append(str(my_dict['txid']))  
            temp.append(str(my_dict['tags'][0].capitalize()))  
            
            temp.append(str(format_time(my_dict['blocktime'])))  
            temp.append(str(my_dict['blockheight']))  
      ...
      # Configure style and word wrap
        ...
        table = Table(data2,repeatRows=1, colWidths=(87*mm, 37*mm, 47*mm,37*mm))        
        style = TableStyle([
